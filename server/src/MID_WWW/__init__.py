...
from authentication import authorized, midauthorized
import util
import datetime
import json
import MP_T_hack
...
@route('/resources/MID', method=["GET"])
@midauthorized()
def mid_config():
    mid_name = request.params.get("mid_name", None)
...
def buildAllPressureJSON(mid_name):
    ...
    for tr in rows:
        retVal += [{"type": "pressure", "device_type": "sensor unit",
                    "sensor_id": tr["sensor_id"],
                    "addy": tr["address"], "port": tr["port"],
                    "convert": tr["convert_py"], "bias": tr["bias"],
                    "sensor_extra_info": tr["extra_info"],
...
def buildAllBurnerControlsJSON(mid_name):
    ...
    conn = util.getConn()
    # query DB
    extraWhere = reduce(lambda x, y: x + " OR device_type_id=%s",
                        BURNER_DEVICE_IDS[1:],
                        "device_type_id=%s") + " AND year=%s AND port >= 0"
    if mid_name:
        extraWhere += " AND mid_name = %s "

    extraArgs = BURNER_DEVICE_IDS + (CURRENT_YEAR,)
    ...
    rows = util.getRowsFromTable("device",
                                 ...
                                 checkEnabled=True)
    for tr in rows:
        extraWhere = "device_id=%s AND sensor_type_id=%s"
        sensor = util.getRowsFromTable("sensor", extraWhere=extraWhere, extraArgs=(tr["id"], PV_ID), checkEnabled=True,
                                       conn=conn)  # PV sensors for device
        if sensor:
            retVal += [{"type": PV_ID, "device_type": tr["device_type_id"],
                        "sensor_id": sensor[0]["id"], "addy": tr["address"],
                        "mid_name": tr["mid_name"],
                        "bin_id": tr["bin_id"], "bin_sec_id": tr["bin_section_id"]}]
        sensor = util.getRowsFromTable("sensor", extraWhere=extraWhere, extraArgs=(tr["id"], SP_ID), checkEnabled=True,
                                       conn=conn)  # SP sensors for device
        if sensor:
            retVal += [{"type": SP_ID, "device_type": tr["device_type_id"],
                        "sensor_id": sensor[0]["id"], "addy": tr["address"],
                        "mid_name": tr["mid_name"],
                        "bin_id": tr["bin_id"], "bin_sec_id": tr["bin_section_id"]}]
    conn.close()
...
def buildAllAB_VFD_specialJSON(mid_name):
    ...
    for tr in rows:
        retVal += [{"type": "P700", "device_type": "P700",
                    "sensor_id": (tr['sensor_id'], tr['sensor_id2']),
                    "name": tr["device_name"],
                    "mid_name": tr["mid_name"],
                    "bin_id": tr["bin_id"], "bin_sec_id": tr["bin_section_id"]}]
    return retVal
...
@route('/resources/MIDalt', method=["GET"])
@midauthorized()
def mid_config():
    mid_name = request.params.get("mid_name", None)
