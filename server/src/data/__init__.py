...
@route('/resources/data/csv_download', method=["GET"])
@authorized('User')
def cvs_download():
    ...
    bin_ids = request.params.get("bin_ids")
    bin_section_ids = request.params.get("bin_section_ids")
    read_type_ids = request.params.get("read_type_ids")
    ...
    sql = ["SELECT '\"'||to_char(timezone('UTC', r.datetime), 'MM/DD/YYYY HH24:MI:SS')||' UTC\",\"'||b.name||'\",\"'||bs.name||'\",\"'||rt.name||'\",'||rd.avg_value "]
...
AND rt.id = rd.read_type_id ORDER BY b.name, bs.name, rt.name, r.datetime")
    ...
    f.close()
    response.headers['Content-disposition'] = 'attachment; filename=csvdata'+tstamp+'.zip'
