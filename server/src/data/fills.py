...
from authentication import authorized, alarmsauthorized
import util
from subscription import handler as subscription_handler
...
@route('/resources/data/fill_config', method=["GET"])
@authorized('User')
...
@route('/resources/data/fills', method=["GET"])
@authorized('User')
def fill_list():
    ...
    begin_datetime = util.getDateFromParam(request.params.get("begin_datetime"))
    end_datetime = util.getDateFromParam(request.params.get("end_datetime"))
    during_datetime = util.getDateFromParam(request.params.get("during_datetime"))
    ...
    if during_datetime:
        ...
        extra_args += (during_datetime, during_datetime, during_datetime, during_datetime, during_datetime, during_datetime)
    if begin_datetime:
    ...
    return {'xlink': ['/resources/data/fills/' + str(fill_id) for fill_id in ids]}


@route('/resources/data/fill_number', method=["GET"])
@authorized('User')
...
@route('/resources/data/fill_types', method=["GET"])
@authorized('User')
def fill_types_get():
    # TODO: Cache
...
@route('/resources/data/fills', method=["POST"])
...
def fill_new():
    ...
    start_datetime = util.getDateFromParam(request.params.get("air_begin_datetime"))
...
@route('/resources/data/fills/:fill_id#[0-9]+#', method=["GET"])
@authorized('User')
...
@route('/resources/data/fills-fast', method=["GET"])
...
@route('/resources/data/fills/:fill_id#[0-9]+#', method=["PUT"])
@authorized('Fill User')
def fill_update(fill_id):
    ...
    # get parameter values
    fill_number = request.params.get("fill_number", None)
    fill_type_id = request.params.get("fill_type_id", None)

    air_begin_datetime = util.getDateFromParam(request.params.get("air_begin_datetime"))
    emptied_datetime = request.params.get('emptied_datetime')
    if emptied_datetime == 'empty':
        emptied_datetime = "empty"
    else:
        emptied_datetime = util.getDateFromParam(emptied_datetime)

    if request.params.get('filled_datetime') == "empty":
        filled_datetime = "empty"
    else:
        filled_datetime = util.getDateFromParam(request.params.get("filled_datetime"))

    if request.params.get("air_end_datetime") == "empty":
        air_end_datetime = "empty"
    else:
        air_end_datetime = util.getDateFromParam(request.params.get("air_end_datetime"))

    rotation_number = request.params.get("rotation_number", None)
    bin_id = request.params.get("bin_id", None)
    hybrid_code = request.params.get("hybrid_code", None)
    field_code = request.params.get("field_code", None)
    truck = request.params.get("truck", None)
    lot_number = request.params.get("lot_number", None)
    storage_bin_number = request.params.get("storage_bin_number", None)
    storage_bin_code = request.params.get("storage_bin_code", None)
    pre_mc_data = request.params.get("pre_mc", None)
    post_mc_data = request.params.get("post_mc", None)
    during_mc_data = request.params.get("during_mc", None)
    during_mc_date_data = request.params.get("during_mc_date", None)
    roll_datetime = request.params.get('roll_datetime', None)
    bushels = request.params.get("bushels", None)
    depth = request.params.get("depth", None)
    extras = request.params.get("extras", None)
    ...
    if sheller_windows:
        ...
        for sheller_window in sheller_windows:
            ...
            cur.execute('INSERT INTO fill_sheller_window (fill_id, bin_id, bin_section_id, begin_datetime, end_datetime) ' +
    ...
    cur.close()
    conn.close()
    return HTTPResponse(output="Fill updated", status=204)


@route('/resources/data/fills/:fill_id#[0-9]+#', method=["DELETE"])
@authorized('Fill User')
def fill_delete(fill_id):
    ...
    return HTTPResponse(output="Fill deleted.", status=204)
            ...
            if m:
                ...
                try:
                    ...
                    cur.execute("""SELECT avg(rds.avg_value) FROM reading_data_subsample rds, reading_subsample rs WHERE
                                ...
                                rs.datetime < %s""",
                                (replacement['bin_id'], replacement['bin_section_id'], READ_TYPE_WETBULB, start_datetime, end_datetime))
                    row = cur.fetchone()
                    if row:
                        value = row[0]
                finally:
...
@route('/resources/data/report/fill_report', method=["GET"])
@authorized('User')
@noCache()
def fill_report():
    year = request.params.get("year", None)
    begin_fill_number = request.params.get("begin_fill_number", None)
    end_fill_number = request.params.get("end_fill_number", None)
    report_type = request.params.get('type')
    report_form = request.params.get('form')
    ...
    try:
        display_tz = pytz.timezone(request.params.get("display_tz"))
        display_tz_str = request.params.get("display_tz")
    except:
    ...
    for row in rows:
        ...
        if 'reports' in general['configs'] and 'avg_wetbulb' in general['configs']['reports'] and \
            ...
            avg_wetbulb = get_avg_wetbulb(conn, row["bin_id"], row["air_begin_datetime"], row["air_end_datetime"], bin_number, general['configs'])
        ...
        avg = {"up_time", "up_percent", "down_time", "down_percent", "pre_avg", "post_avg", "mc_removed", "bushels",
               "hrs_per_point"}
        group = []
        groups = []
        avgs = []
    ...
    if report_type == 'fill' and report_form == 'full':
        ...
        #    if not fill[key]:
        #        fill[key] = '&#xA0;'
        return template('fill_report', report_title=report_title, fills=fills, general=general,
                        ts=long(time.time() * 1000),
                        display_tz=urllib.quote(display_tz_str, ''))
        ...
        return template('fill_tab', report_title="Yearly Report", fills=fills, general=general, timestamp=now_str,
                        totals=totals,
                        avgs=avgs)
        ...
        if 'reports' in general['configs'] and 'csv_reversal_snapshot' in general['configs']['reports']:
            ...
            extra_column_map = add_reversal_snapshot(conn, general['configs']['reports']['csv_reversal_snapshot'], fills, addons)
        fills_to_string(fills, report_form, display_tz)
        ...
        return template('fill_hybrid_tab', report_title="Yearly Hybrid Report", groups=groups, general=general,
                        groupTotals=group_totals, groupAvgs=group_avgs, totals=totals, avgs=avgs, timestamp=now_str)
            ...
            if len(rows) > 0:
                ...
                for row in rows:
                    ...
                    column_map[key] = 'Reversal Snapshot ' + row['bin_section_name'] + \
                                      ' ' + row['short_name'] + '(' + string.replace(row['units'], ';', '') + ')'
            cur.close()
...
def group_to_hybrid(fills):
    groups = []
    sfills = sorted(fills, cmp=fill_hybrid_number_sort)
    current_hybrid = -1
    for fill in sfills:
        hybrid = fill["hybrid"]
        if hybrid:
...
def csv_fill(fills, general, fname, extra_column_map):
    ...
    response.headers['Content-disposition'] = 'attachment; filename='+fname+'.csv'
...
@route('/resources/data/sheller_sensor_mc', method=['GET'])
@authorized('User')
@noCache()
