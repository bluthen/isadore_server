...
@route('/resources/data/graphp', method=["GET"])
@authorized('User')
@noCache()
def graphp():
    fill_id = request.params.get('fill_id', None)
    if not fill_id:
...
@route('/resources/data/graphq', method=["GET"])
@authorized('User')
@noCache()
def graphq():
    try:
        bin_id = int(request.params.get("bin_id"))
    except ValueError:
        abort(400, 'Invalid bin_id')
    try:
        query_top = json.loads(request.params.get("query_top"))
    except:
        abort(400, 'invalid query_top')
    try:
        query_bottom = json.loads(request.params.get("query_bottom"))
    except:
        abort(400, 'invalid query_bottom')
    ...
    try:
        ...
        abort(400, 'invalid sample_period parameter')
