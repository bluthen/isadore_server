...
def customJSONHandler(obj):
	'''Handles date objects in the json conversion process.'''
...
def responseJSON(obj):
	...
	return json.dumps(obj, default=customJSONHandler)
...
def getAccount(email, password):
	...
	if (account):
		...
		if (password in account.session_hashes):
			return account
		elif (account.password == hashlib.sha1(account.seed + password).hexdigest()):
			return account
		else:
...
def getAccountByEmail(email):
	...
	if row:
		...
		if session_hashes:
			session_hashes = list(zip(*session_hashes)[0])
		else:
			session_hashes = []
