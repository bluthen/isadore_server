...
class TestSensorData(unittest.TestCase):
    ...
    def _reset_valid(valids):
        for sample, srows in valids.items():
            for vrows in srows:
                vrows['_checked'] = False

    def _verify(self, name, cur, valids, verbose=False):
        cur.execute("SELECT rs.datetime, rs.sample_period, rds.bin_id, rds.bin_section_id, rds.read_type_id, \
        rds.avg_value, rds.min, rds.max, rds.current_count FROM reading_data_subsample rds, reading_subsample rs \
        WHERE rs.id = rds.reading_subsample_id")
        for row in cur:
            ...
            if sample_period in valids:
                for vrows in valids[sample_period]:
                    if row['datetime'] == vrows['datetime'] and row['bin_id'] == vrows['bin_id'] and \
                        ...
                        for key, val in vrows.items():
                            if not key.startswith('_'):
                                if verbose and key == 'avg_value':
                                    print(row['read_type_id'], row[key], val)
                                self.assertAlmostEqual(row[key], val, 7, '%s: %s: %s != %s, row=%s' % (
                                    name, key, str(row[key]), str(val), str(row)))
    ...
    def test_simple_subsample(self):
        ...
        try:
            ...
            cur.execute("SELECT rs.datetime, rs.sample_period, rds.bin_id, rds.bin_section_id, rds.read_type_id, \
            rds.avg_value, rds.min, rds.max, rds.current_count FROM reading_data_subsample rds, \
            reading_subsample rs WHERE rs.id = rds.reading_subsample_id")
            # TODO: Add other sample tests
            valids = {'5': [
                {'datetime': iso8601.parse_date('1980-01-01T12:05:00-06:00'), 'bin_id': 16, 'bin_section_id': 14,
                 'read_type_id': 10, 'avg_value': 70.0, 'min': 70.0, 'max': 70.0, 'current_count': 1,
                 "_checked": False}], '10': [
                {'datetime': iso8601.parse_date('1980-01-01T12:10:00-06:00'), 'bin_id': 16, 'bin_section_id': 14,
                 'read_type_id': 10, 'avg_value': 70.0, 'min': 70.0, 'max': 70.0, 'current_count': 1,
                 "_checked": False}]}
            ...
            valids = {
                '5': [
                    {'datetime': iso8601.parse_date('1980-01-01T12:05:00-06:00'), 'bin_id': 16, 'bin_section_id': 14,
                     ...
                     "_checked": False},
                    {'datetime': iso8601.parse_date('1980-01-01T12:10:00-06:00'), 'bin_id': 16, 'bin_section_id': 14,
                     'read_type_id': 10, 'avg_value': 72.0, 'min': 72.0, 'max': 72.0, 'current_count': 1,
                     "_checked": False}
                ],
                '10': [
                    {'datetime': iso8601.parse_date('1980-01-01T12:10:00-06:00'), 'bin_id': 16, 'bin_section_id': 14,
                     'read_type_id': 10, 'avg_value': 71.0, 'min': 70.0, 'max': 72.0, 'current_count': 2,
                     "_checked": False}
                ]
            }
    ...
    def test_calc_guage_pressure(self):
        ...
        try:
            ...
            cur.execute('INSERT INTO sensor_data(sensor_id, value, datetime) VALUES (%s, %s, %s)',
                        (320, 97.5, '1980-01-01T18:07:00Z'))
            valids = {'5': [
                {'datetime': iso8601.parse_date('1980-01-01T12:05:00-06:00'), 'bin_id': 16, 'bin_section_id': 14,
                 'read_type_id': 14, 'avg_value': 97.5, 'min': 97.5, 'max': 97.5, 'current_count': 1,
                 "_checked": False},
                {'datetime': iso8601.parse_date('1980-01-01T12:05:00-06:00'), 'bin_id': 9, 'bin_section_id': 9,
                 'read_type_id': 14, 'avg_value': 96.5, 'min': 96.5, 'max': 96.5, 'current_count': 1,
                 "_checked": False},
                {'datetime': iso8601.parse_date('1980-01-01T12:05:00-06:00'), 'bin_id': 16, 'bin_section_id': 14,
                 'read_type_id': 7, 'avg_value': 4.0186, 'min': 4.0186, 'max': 4.0186, 'current_count': 1,
                 "_checked": False}]}
            self._verify('calc_guage_pressure', cur, valids)

            # insert pressure into a bins time2
            cur.execute('INSERT INTO sensor_data(sensor_id, value, datetime) VALUES (%s, %s, %s)',
                        (321, 97.5, '1980-01-01T19:07:00Z'))
            cur.execute('INSERT INTO sensor_data(sensor_id, value, datetime) VALUES (%s, %s, %s)',
                        (320, 98.5, '1980-01-01T19:07:00Z'))
            # insert pressure into outdoor time2
            cur.execute('INSERT INTO sensor_data(sensor_id, value, datetime) VALUES (%s, %s, %s)',
                        (299, 96.5, '1980-01-01T19:07:00Z'))
            valids = {'5': [
                {'datetime': iso8601.parse_date('1980-01-01T13:05:00-06:00'), 'bin_id': 16, 'bin_section_id': 14,
                 'read_type_id': 14, 'avg_value': 98.5, 'min': 98.5, 'max': 98.5, 'current_count': 1,
                 "_checked": False},
                {'datetime': iso8601.parse_date('1980-01-01T13:05:00-06:00'), 'bin_id': 16, 'bin_section_id': 13,
                 'read_type_id': 14, 'avg_value': 97.5, 'min': 97.5, 'max': 97.5, 'current_count': 1,
                 "_checked": False},
                {'datetime': iso8601.parse_date('1980-01-01T13:05:00-06:00'), 'bin_id': 9, 'bin_section_id': 9,
                 'read_type_id': 14, 'avg_value': 96.5, 'min': 96.5, 'max': 96.5, 'current_count': 1,
                 "_checked": False},
                {'datetime': iso8601.parse_date('1980-01-01T13:05:00-06:00'), 'bin_id': 16, 'bin_section_id': 14,
                 'read_type_id': 7, 'avg_value': 8.0372, 'min': 8.0372, 'max': 8.0372, 'current_count': 1,
                 "_checked": False},
                {'datetime': iso8601.parse_date('1980-01-01T13:05:00-06:00'), 'bin_id': 16, 'bin_section_id': 13,
                 'read_type_id': 7, 'avg_value': 4.0186, 'min': 4.0186, 'max': 4.0186, 'current_count': 1,
                 "_checked": False}
    ...
    def test_calc_wetbulb(self):
        ...
        try:
            ...
            valids = {'5': [
                {'datetime': iso8601.parse_date('1980-01-01T12:05:00-06:00'), 'bin_id': 16, 'bin_section_id': 14,
                 'read_type_id': 131, 'avg_value': 94.04024, 'min': 94.04024, 'max': 94.04024, 'current_count': 1,
                 "_checked": False}
            ]}
            ...
            inserts.append("INSERT INTO sensor_data(sensor_id, value, datetime) VALUES (%s, %s, '%s')" % (
                316, 99.0, '1980-01-01T18:08:00Z'))
            ...
            valids = {'5': [
                {'datetime': iso8601.parse_date('1980-01-01T12:05:00-06:00'), 'bin_id': 16, 'bin_section_id': 14,
                 ...
                 "_checked": False},
                {'datetime': iso8601.parse_date('1980-01-01T12:10:00-06:00'), 'bin_id': 16, 'bin_section_id': 14,
                 'read_type_id': 10, 'avg_value': 99.0, 'min': 99.0, 'max': 99.0, 'current_count': 1,
                 "_checked": False}],
                '10': [{'datetime': iso8601.parse_date('1980-01-01T12:10:00-06:00'), 'bin_id': 16, 'bin_section_id': 14,
                        'read_type_id': 131, 'avg_value': 93.56540000000004, 'min': 93.56540000000004,
                        'max': 93.56540000000004, 'current_count': 1, "_checked": False}]
            }
    ...
    def test_sensor_mirror(self):
        ...
        try:
            ...
            valids = {
                '5': [
                    {'datetime': iso8601.parse_date('1980-01-01T12:05:00-06:00'), 'bin_id': 15, 'bin_section_id': 14,
                     'read_type_id': 10, 'avg_value': 70.0, 'min': 70.0, 'max': 70.0, 'current_count': 1,
                     "_checked": False},
                    {'datetime': iso8601.parse_date('1980-01-01T12:10:00-06:00'), 'bin_id': 15, 'bin_section_id': 14,
                     'read_type_id': 10, 'avg_value': 72.0, 'min': 72.0, 'max': 72.0, 'current_count': 1,
                     "_checked": False},
                    {'datetime': iso8601.parse_date('1980-01-01T12:05:00-06:00'), 'bin_id': 201, 'bin_section_id': 14,
                     'read_type_id': 10, 'avg_value': 70.0, 'min': 70.0, 'max': 70.0, 'current_count': 1,
                     "_checked": False},
                    {'datetime': iso8601.parse_date('1980-01-01T12:10:00-06:00'), 'bin_id': 201, 'bin_section_id': 14,
                     'read_type_id': 10, 'avg_value': 72.0, 'min': 72.0, 'max': 72.0, 'current_count': 1,
                     "_checked": False}
                ],
                '10': [
    ...
    def test_bin_section_group(self):
        ...
        try:
            cur.execute("INSERT INTO sensor_data(sensor_id, value, datetime) VALUES (%s, %s, %s)",
                        (312, 71.0, '1980-01-01T18:07:00Z'))  # Bin 4 Bottom Temp
            cur.execute("INSERT INTO sensor_data(sensor_id, value, datetime) VALUES (%s, %s, %s)",
                        (324, 72.0, '1980-01-01T18:07:00Z'))  # Bin 4 Pos 1 MT
            cur.execute("INSERT INTO sensor_data(sensor_id, value, datetime) VALUES (%s, %s, %s)",
                        (322, 73.0, '1980-01-01T18:07:00Z'))  # Bin 5 Pos 1 MT
            cur.execute("INSERT INTO sensor_data(sensor_id, value, datetime) VALUES (%s, %s, %s)",
                        (323, 74.0, '1980-01-01T18:07:00Z'))  # Bin 5 Pos 2 MT
            valids = {
                '5': [
                    {'datetime': iso8601.parse_date('1980-01-01T12:05:00-06:00'), 'bin_id': 15, 'bin_section_id': 14,
                     'read_type_id': 10, 'avg_value': 71.5, 'min': 71.0, 'max': 72.0, 'current_count': 2,
                     "_checked": False},
                    {'datetime': iso8601.parse_date('1980-01-01T12:05:00-06:00'), 'bin_id': 15, 'bin_section_id': 200,
                     'read_type_id': 10, 'avg_value': 72.0, 'min': 72.0, 'max': 72.0, 'current_count': 1,
                     "_checked": False},
                    {'datetime': iso8601.parse_date('1980-01-01T12:05:00-06:00'), 'bin_id': 16, 'bin_section_id': 200,
                     'read_type_id': 10, 'avg_value': 73.0, 'min': 73.0, 'max': 73.0, 'current_count': 1,
                     "_checked": False},
                    {'datetime': iso8601.parse_date('1980-01-01T12:05:00-06:00'), 'bin_id': 16, 'bin_section_id': 201,
                     'read_type_id': 10, 'avg_value': 74.0, 'min': 74.0, 'max': 74.0, 'current_count': 1,
                     "_checked": False},
                    {'datetime': iso8601.parse_date('1980-01-01T12:05:00-06:00'), 'bin_id': 16, 'bin_section_id': 14,
                     'read_type_id': 10, 'avg_value': 73.5, 'min': 73.0, 'max': 74.0, 'current_count': 2,
                     "_checked": False}
                ]
            }
    ...
    def test_bin_group(self):
        ...
        try:
            cur.execute("INSERT INTO sensor_data(sensor_id, value, datetime) VALUES (%s, %s, %s)",
                        (302, 71.0, '1980-01-01T18:07:00Z'))  # Bin 1 Bottom Temp
            cur.execute("INSERT INTO sensor_data(sensor_id, value, datetime) VALUES (%s, %s, %s)",
                        (308, 72.0, '1980-01-01T18:07:00Z'))  # Bin 3 Bottom Temp
            valids = {
                '5': [
                    {'datetime': iso8601.parse_date('1980-01-01T12:05:00-06:00'), 'bin_id': 12, 'bin_section_id': 14,
                     'read_type_id': 10, 'avg_value': 71.0, 'min': 71.0, 'max': 71.0, 'current_count': 1,
                     "_checked": False},
                    {'datetime': iso8601.parse_date('1980-01-01T12:05:00-06:00'), 'bin_id': 14, 'bin_section_id': 14,
                     'read_type_id': 10, 'avg_value': 72.0, 'min': 72.0, 'max': 72.0, 'current_count': 1,
                     "_checked": False},
                    {'datetime': iso8601.parse_date('1980-01-01T12:05:00-06:00'), 'bin_id': 200, 'bin_section_id': 14,
                     'read_type_id': 10, 'avg_value': 71.5, 'min': 71.0, 'max': 72.0, 'current_count': 2,
                     "_checked": False}
                ]
            }
    ...
    def test_error_status_no_reading(self):
        ...
        try:
            inserts = [
                ...
                VALUES (316, 500.0, '1980-01-01T18:07:00Z', 1)"
            ]

            valids = {'5': [
                {'datetime': iso8601.parse_date('1980-01-01T12:05:00-06:00'), 'bin_id': 16, 'bin_section_id': 14,
                 'read_type_id': 10, 'avg_value': 100.0, 'min': 100.0, 'max': 100.0, 'current_count': 1,
                 "_checked": False}
            ]}
