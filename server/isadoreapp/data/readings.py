...
from isadoreapp.authentication import authorized, midauthorized, alarmsauthorized
import datetime
...
@app.route('/resources/data/sensor/latesterrorcodecount', methods=["GET"])
def sensor_last_error_code_count():
    sql = "SELECT count(error_code) FROM sensor_data WHERE error_code IS NOT NULL and \
    ...
    if row:
        ...
        for row in cur:
            error_info.append({"bin_name": row[0],
                               ...
                               "port": row[4],
                               "address": row[5]})
    return jsonify({"error_code_count": count, "info": error_info})


# Don't require authorization for xymon
@app.route('/resources/data/readings/lasttimedelta', methods=["GET"])
def readings_last_time_delta():
    ...
    conn = util.getConn()
    cur = conn.cursor()
    cur.execute('SELECT extract(epoch from (now()-datetime))/60 from reading_subsample WHERE sample_period = %s \
    ORDER by datetime desc limit 1', (sample_period,))
    row = cur.fetchone()
    if not row:
        cur.close()
        conn.close()
    ...
    return jsonify({'td': minutes, "info": error_info})
    

@app.route('/resources/data/readings/last', methods=["GET"])
...
def readings_last():
    ...
    cur = conn.cursor()
    # perform SQL query
    cur.execute("SELECT id, datetime from reading_subsample WHERE sample_period = %s \
    ORDER BY datetime desc limit 2", (sample_period,))
    ...
    countdown = (dt + datetime.timedelta(seconds=grow['interval']) - datetime.datetime.now(dt.tzinfo) +
    ...
    return jsonify({'xlink': xlinks,
                              'datetime': d,
                              'countdown': countdown})


@app.route('/resources/data/readings/<int:reading_id>', methods=["GET"])
@alarmsauthorized('User')
def reading_get_data(reading_id):

    # get parameter values
    bin_id = request.values.get("bin_id")
    bin_section_id = request.values.get("bin_section_id")
    read_type_id = request.values.get("read_type_id")
    
    extra_where = " reading_subsample_id = %s"
    extra_args = (reading_id,)
    ...
    return jsonify({"results": reading_data})


@app.route('/resources/data/readings/latest', methods=['GET'])
@authorized('User')
def readings_data_latest(conn=None):
    return jsonify(r_readings_data_latest(conn))
...
@app.route('/resources/data/sensor_data_latest', methods=["GET"])
@authorized('User')
def sensor_data_latest():
    sensor_ids = request.values.get("sensor_ids")
    try:
...
@app.route('/resources/data/diagnostics_sensor_data_latest_reset', methods=["DELETE"])
...
def diag_sensor_data_latest_reset():
    ...
    cur.close()
    conn.close()
    return Response(response="Sensor Data Latest Reset", status=204)


@app.route('/resources/data/diagnostics_sensor_data_latest', methods=["GET"])
@authorized('User')
def diag_sensor_data_latest():
    return jsonify({"results": r_diag_sensor_data_latest()})
...
def r_diag_sensor_data_latest(con=None):
    ...
    d.name as device_name, d.info as device_info, dt.name as device_type_name, d.mid_name, d.port, d.address, 
    d.bin_id, d.bin_section_id, b.name as bin_name, bs.name as bin_section_name
FROM
...
@app.route('/resources/data/sensor', methods=["POST"])
@midauthorized()
def data_new():
    ...
    try:
        ...
        logging.exception('Bad parameters, json: %s' % (json.dumps(request.values.get('data'))))
    ...
    try:
        ...
        return Response(response="Data added.", status=204)
    finally:
