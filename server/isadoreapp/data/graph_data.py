...
import json
import time
from isadoreapp.alarms import EMC
...
@app.route('/resources/data/graph/selections/<int:selections_id>', methods=["GET"])
@authorized('User')
# @confCache()
def data_graph_selections(bin_id):
    true_bin = False
    selections = {}
    tops = []

    conn = util.getConn()
    binr = util.getRowFromTableById("bin", int(bin_id), conn=conn)
    bin_sections = []
    bin_sections_names = []
    if not binr:
...
def dummy_graph_data(bin_id, query, sample_period, begin_datetime, end_datetime):
    ...
    return flask.send_from_directory('./gen_data', filename, mimetype='application/json')


@app.route('/resources/data/graph/data', methods=["GET"])
@authorized('User')
def data_graph_data():
    """
query format
...
"""
    ...
    begin_datetime = util.getDateFromParam(request.values.get('begin_datetime', None))
    end_datetime = util.getDateFromParam(request.values.get('end_datetime', None))
    sample_period = util.getRequiredIntParam('sample_period')
    query_param = json.loads(request.values.get('query'))

    return jsonify(graph_data_struct(bin_id, query_param, sample_period, begin_datetime, end_datetime))
...
def graph_data_struct(bin_id, query, sample_period, begin_datetime, end_datetime):
    ...
    try:
        max_temp_idx = list(zip(*query))[0].index('maxtemp')
        if len(data) > 0:
            times = list(zip(*data))[0]
            max_temps = list(map(lambda t: [None, t, None], EMC.compute_max_temps_multi(times, bin_id)))
