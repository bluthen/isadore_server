...
class AlarmWatcher(Daemon):
    ...
    def has_global_alert(self, alarm_type_id, cur):
    ...
    def send_alarm_notice(self, short_name, alarm_id, alarm_type_id, event_time, conn, msg):
        cur = conn.cursor()
        ...
        if alarm_type_id == self.ALARM_TYPE_SENSOR_TEMP:
            ...
            if email:
                try:
                    ...
                    util.sendEmail(email, "info@example.com", "Isadore Alarm: Temperature Condition", amsg)
                except:
    ...
    def send_global_notices(self, short_name, alarm_type_id, event_time, conn):
        cur = conn.cursor()
            ...
            if emails:
                for email in emails:
                    try:
                        ...
                        util.sendEmail(email, "info@example.com", "Isadore Alarm: MID Down", msg)
                    except:
                        logging.error(traceback.format_exc())
            if sms:
    ...
    def run(self):
        ...
        while True:
            try:
                ...
                row = cur.fetchone()
                read_delta_alt = now - row[1]
                row = cur.fetchone()
                working_last_read = row[1]
                working_last_read_id = row[0]

                read_delta = now - working_last_read
                mid_global_event = self.has_global_alert(self.ALARM_TYPE_MID_DOWN, cur)
                logging.debug("read_delta: " + str(read_delta))
                if read_delta_alt > datetime.timedelta(seconds=(20 * 60.0)):
                    if not mid_global_event:
                        util.insertRow("alarm_global_event",
                                       {"alarm_type_id": self.ALARM_TYPE_MID_DOWN, "begin_datetime": now}, cur)
                        conn.commit()
                        self.send_global_notices(general_config["customer_short_name"], self.ALARM_TYPE_MID_DOWN, now,
                                                 conn)
                elif mid_global_event:
                    # MID is back up.
                    util.updateRowById("alarm_global_event", mid_global_event, {"end_datetime": now})
                    conn.commit()
                ...
                if working_last_read > last_read:
                    ...
                    temp_rows = util.getRowsFromTable("alarm", extraWhere=" alarm_type_id = %s ",
                    ...
                    for row in temp_rows:
                        ...
                        if row["greater_than_p"]:
                            ...
                            gtlt = "<"
                        aironsql = ""
                        if "alarms" in gc_configs and "aironly" in gc_configs["alarms"] and \
                                gc_configs["alarms"]["aironly"]:
                            aironsql = " AND bin_fill_airon(b.id) IS TRUE "

                        if "alarms" in gc_configs and "bottom_only" in gc_configs["alarms"] and \
...
reading_data_subsample rd, reading_data_subsample rd2, bin b WHERE
  ...
  rd2.bin_section_id = 14 and greatest(rd.avg_value, rd2.avg_value) """ + gtlt + """ %s"""
                            cur.execute(sql,
                                        (working_last_read_id, working_last_read_id, self.READ_TYPE_TEMP, row["value"]))
                            rows = cur.fetchall()
                            count_row = [len(rows)]
                        if count_row[0] > 0 and not ae_id:
                            sql = "INSERT INTO alarm_event (alarm_id, begin_datetime) VALUES (%s, %s)"
                            cur.execute(sql, (row["id"], now))
                            conn.commit()
                            self.send_alarm_notice(general_config["customer_short_name"], row["id"],
                                                   self.ALARM_TYPE_SENSOR_TEMP, now, conn,
                                                   gtlt + " %.2f" % row["value"])
                            # TODO: contact
                        elif count_row[0] == 0 and ae_id:
