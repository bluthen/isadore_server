    ...
    def sendAlarmNotices(self):
        ...
        if(emails):
            for email in emails:
                ...
                util.sendEmail(email, "info@example.com", self.subject, self.msg)
        if sms:
...
class UserAlarm(Alarm):
    ...
    def sendAlarmNotice(self, alarm_id, alarmMSG):
        ...
        if(email):
            ...
            util.sendEmail(email, "info@example.com", self.subject, amsg)
        if(sms):
...
ORDER BY bin.name";
            ...
            if(bins):
                ...
                for bin in bins:
                    ...
                    mcTemp = EMC.compute_max_temps(bin[3], bin[2], deductions, self.reading_datetime, self.general_config["emchrs_per_point"])
